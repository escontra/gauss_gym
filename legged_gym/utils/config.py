...
from legged_gym.utils import path
  ...
  def _ensure_values(self, mapping):
    ...
    for key, value in result.items():
      ...
      if isinstance(value, tuple):
        if len(value) == 0:
          message = f'Key: {key}:Empty lists are disallowed because their type is unclear.'
          raise TypeError(message)
        if not isinstance(value[0], (str, float, int, bool, list)):
          message = f'Key: {key}: Lists can only contain strings, floats, ints, bools'
          message += f' but not {type(value[0])}'
          raise TypeError(message)
        if not all(isinstance(x, type(value[0])) for x in value[1:]):
          message = f'Key: {key}: Elements of a list must all be of the same type.'
          raise TypeError(message)
      result[key] = value
    return result
