...
class RayCaster():
    def __init__(self,
        ...
        self.ray_directions = math.quat_apply(offset_quat.repeat(np.prod(self.ray_directions.shape[:-1]), 1), self.ray_directions)
    ...
    def update(self, dt, env_ids=...):
        ...
        if self.attach_yaw_only:
            ray_starts_world = math.quat_apply_yaw(quats, self.ray_starts[env_ids]) + pos
            ray_directions_world = self.ray_directions[env_ids]
        else:
            ray_starts_world = math.quat_apply(quats, self.ray_starts[env_ids]) + pos
            ray_directions_world = math.quat_apply(quats, self.ray_directions[env_ids])

        self.ray_hits_world[env_ids] = warp_utils.ray_cast(ray_starts_world, ray_directions_world, self.terrain_mesh)
    ...
    def debug_vis(self, env):
        ...
        if self.env.selected_environment >= 0:
          ...
          self.sphere_geom.draw(ray_hits_world_viz, env.gym, env.viewer, env.envs[0], only_render_selected=only_render_selected_range)
        else:
...
class GaussianSplattingRenderer():
    ...
    def update(self, dt, env_ids=...):
        ...
        cam_rot = math.quaternion_to_matrix(cam_quat)
...
class LinkHeightSensor():
    def __init__(self, env, link_names, color=(1, 0, 0), attach_yaw_only=True):        
        ...
        offset_pos = torch.tensor(list(self.attachement_pos), device=self.device)
        offset_quat = torch.tensor(list(self.attachement_quat), device=env.device)
        self.ray_directions = math.quat_apply(offset_quat.repeat(len(self.ray_directions), 1), self.ray_directions)
        self.ray_starts += offset_pos
