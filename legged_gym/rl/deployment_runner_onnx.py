...
class DeploymentRunner:
  def __init__(self, cfg: Dict, model_name: str, execution_provider: str = 'CPUExecutionProvider', num_envs: int = 1):
    self.cfg = cfg
  ...
  def load(self, resume_root: pathlib.Path):
    ...
    self.onnx_init_hidden_states = [h.repeat(self.num_envs, 1) for h in self.onnx_init_hidden_states]
    self.onnx_hidden_states = np.copy(self.onnx_init_hidden_states)

  def get_input_dict(self, obs_dict: Dict[str, np.ndarray], hidden_states: List[np.ndarray]) -> Dict[str, np.ndarray]:
    return {
      **obs_dict,
      **{f"hidden_{i}": h for i, h in enumerate(hidden_states)},
    }

  def process_outputs(self, outputs: Dict[str, np.ndarray], rnn_only: bool) -> Tuple[Dict[str, np.ndarray], np.ndarray, List[np.ndarray]]:
    if rnn_only:
      model_preds = {}
    else:
  ...
  def predict(self, obs: Dict[str, np.ndarray], rnn_only: bool = False) -> Tuple[Dict[str, np.ndarray], np.ndarray]:
    ...
    model_preds, rnn_state, self.onnx_hidden_states = self.process_outputs(outputs, rnn_only)
