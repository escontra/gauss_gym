...
import gauss_gym
from gauss_gym.utils import flags, config, helpers, observation_groups, observations
from gauss_gym.rl.deployment_runner_onnx import DeploymentRunner
    ...
    for observation in group.observations:
      if observation.name == 'projected_gravity':
        ...
        obs = observations.quat_rotate_inverse_np(
          quat[None], np.array([0.0, 0.0, -1.0], dtype=np.float32)[None]
        )[0]
...
def clip_by_torque_limit(
  actions_scaled, torque_limits, dof_pos, dof_vel, p_gains, d_gains, default_dof_pos
):
...
def main(argv=None):
  ...
  mj_model = mujoco.MjModel.from_xml_path(
    cfg['asset']['mujoco_file'].format(GAUSS_GYM_ROOT_DIR=gauss_gym.GAUSS_GYM_ROOT_DIR)
  )
  ...
  print(
    f'{"MJ Index":<10}{"IG Index":<10}{"Name":<20}{"Default Pos":<15}{"Stiffness":<15}{"Damping":<15}'
  )
  print(
    f'{"-" * 10:<10}{"-" * 10:<10}{"-" * 20:<20}{"-" * 15:<15}{"-" * 15:<15}{"-" * 15:<15}'
  )
  for i in range(mj_model.nu):
    mj_name = mujoco.mj_id2name(mj_model, mujoco.mjtObj.mjOBJ_ACTUATOR, i)
    ...
    print(
      f'{i:<10}{ig_idx:<10}{mj_name:<20}{default_dof_pos_mj[i]:<15.1f}{dof_stiffness_mj[i]:<15.1f}{dof_damping_mj[i]:<15.1f}'
    )
  ...
  mj_data.qpos = np.concatenate(
    [
      ...
      np.array(
        cfg['init_state']['rot'][3:4] + cfg['init_state']['rot'][0:3], dtype=np.float32
      ),
      default_dof_pos_mj,
    ]
  )
  ...
  use_gait_frequency = (
    'GAIT_PROGRESS' in cfg['observations'][cfg['policy']['obs_key']]['observations']
  )
  ...
  with mujoco.viewer.launch_passive(mj_model, mj_data) as viewer:
    ...
    while viewer.is_running():
      ...
      it += 1
      gait_process = float(
        np.fmod(gait_process + cfg['sim']['dt'] * gait_frequency, 1.0)
      )
