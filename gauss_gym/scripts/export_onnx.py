...
from gauss_gym.utils import flags, config, helpers
from gauss_gym.utils.task_registry import task_registry
from gauss_gym.rl.runner import Runner


def export_onnx(
  model_name, model, obs_space, output_names, save_path, test_inputs: bool = False
):
  # Prepare dummy inputs
  utils.print(f'Preparing dummy inputs for {model_name}...')
  dummy_obs = {
    k: torch.ones(v.shape, dtype=v.dtype)[None].to(next(model.parameters()).device)
  ...
  if test_inputs:
    ...
    inference_input_dict = {
      f'{k}': v.repeat(batch_size, *[1 for _ in range(v.ndim - 1)])
      .cpu()
      .numpy()  # Create the prefixed key expected by ONNX
    ...
    for k, v in zip(output_names, outputs):
      utils.print(
        f'\t{k}: shape={v.shape}, min={v.min():.2f}, max={v.max():.2f}, mean={v.mean():.2f}',
        color='blue',
      )
...
def main(argv=None):
  ...
  parsed, other = flags.Flags(
    {'runner': {'load_run': ''}, 'model_name': 'all', 'output_path': 'default'}
  ).parse_known(argv)
  ...
  cfg = cfg.update({'image_encoder.params.torchaug_transforms': False})
  ...
  for model_name in export_models:
    ...
    model.eval()
    export_onnx(
      model_name, model, obs_space, output_names, output_path, test_inputs=True
    )
