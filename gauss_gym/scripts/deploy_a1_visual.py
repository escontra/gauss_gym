...
import gauss_gym
from gauss_gym.utils import flags, config, visualization
...
def get_input_filter(cfg):
  ...
  def input_filter(image: np.ndarray, width: int, height: int):
  ...
  return functools.partial(input_filter, width=new_width, height=new_height)
  ...
  try:
    ...
    while not rospy.is_shutdown():
      ...
      rospy.loginfo_throttle(
        10, 'realsense duration: {:.3f}'.format(realsense_duration)
      )
      ...
      rospy.loginfo_throttle(
        10, 'inference duration: {:.3f}'.format(inference_duration)
      )
      if parsed.debug and frames.is_frame():
        ...
        rgb_image_msg.header.frame_id = parsed.namespace + '/camera_color_optical_frame'
