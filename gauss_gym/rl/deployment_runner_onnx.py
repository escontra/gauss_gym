...
class DeploymentRunner:
  ...
  def load(self, resume_root: pathlib.Path):
    ...
    self.obs_space = pickle.load(
      open(self.resume_path / f'{self.model_name}_obs_space.pkl', 'rb')
    )
    ...
    self.onnx_session = onnxruntime.InferenceSession(
      str(onnx_path), sess_options=sess_options, providers=[self.execution_provider]
    )
    ...
    self.model_pred_keys = self.onnx_output_names[
      : self.onnx_output_names.index(self.rnn_state_key)
    ]
    self.hidden_state_keys = self.onnx_output_names[
      self.onnx_output_names.index(self.rnn_state_key) + 1 :
    ]
    with open(hidden_states_path, 'rb') as f:
      self.onnx_init_hidden_states = pickle.load(f)
    self.onnx_init_hidden_states = [
      h.repeat(self.num_envs, 1) for h in self.onnx_init_hidden_states
    ]
    self.onnx_hidden_states = np.copy(self.onnx_init_hidden_states)

  def get_input_dict(
    self, obs_dict: Dict[str, np.ndarray], hidden_states: List[np.ndarray]
  ) -> Dict[str, np.ndarray]:
    return {
      **obs_dict,
  ...
  def process_outputs(
    self, outputs: Dict[str, np.ndarray], rnn_only: bool
  ) -> Tuple[Dict[str, np.ndarray], np.ndarray, List[np.ndarray]]:
    if rnn_only:
      model_preds = {}
    else:
      model_preds = {k[4:]: outputs[k] for k in self.model_pred_keys}
  ...
  def predict(
    self, obs: Dict[str, np.ndarray], rnn_only: bool = False
  ) -> Tuple[Dict[str, np.ndarray], np.ndarray]:
    input_dict = self.get_input_dict(obs, self.onnx_hidden_states)
    if rnn_only:
    ...
    model_preds, rnn_state, self.onnx_hidden_states = self.process_outputs(
      outputs, rnn_only
    )
    return model_preds, rnn_state

  def reset(self, done: np.ndarray):
    new_hidden_states = []
    for orig_hs, curr_hs in zip(self.onnx_init_hidden_states, self.onnx_hidden_states):
      new_hs = np.copy(curr_hs)
      new_hs[:, done, :] = orig_hs[:, done, :]
      new_hidden_states.append(new_hs)
    self.onnx_hidden_states = tuple(new_hidden_states)
