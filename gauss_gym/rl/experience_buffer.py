...
class MiniBatch:
  ...
  def __repr__(self):
    ...
    for obs_group in self.obs_sym.keys():
      repr_str += f'\n\t{obs_group}:'
      for obs_name in self.obs_sym[obs_group].keys():
        repr_str += f'\n\t\t{obs_name}: {self.obs_sym[obs_group][obs_name].shape}'
    repr_str += '\n\n'
...
class ExperienceBuffer:
  ...
  def add_buffer(self, name, shape, dtype=None, is_hidden_state: bool = False):
    if is_hidden_state:
      if shape is None or shape == (None, None):
        return
      shape = shape[0] if isinstance(shape[0], tuple) else (shape[0],)
  ...
  def update_data(self, name, idx, data, is_hidden_state: bool = False):
    if is_hidden_state:
      if data is None or data == (None, None):
        return
      data = data[0] if isinstance(data[0], tuple) else (data[0],)
  ...
  def _split_and_pad_obs(
    self, obs_key, dones_key, hidden_states_key=None, hidden_states_is_next=False
  ):
    ...
    if hidden_states_key:
      offset_idx = 1 if hidden_states_is_next else 0
      hidden_states = pytree.tree_map(
        lambda x: rl_utils.split_and_pad_trajectories(
          x.permute(0, 2, 1, 3), self.tensor_dict[dones_key]
        )[0]
        .permute(0, 2, 1, 3)[offset_idx]
        .detach(),
        self.tensor_dict[hidden_states_key],
      )

    return obs, traj_masks, hidden_states
  ...
  def reccurent_mini_batch_generator(
    ...
    symms = {}
    if (
      symm_key is not None
      and symm_key in self.tensor_dict
      and self.tensor_dict[symm_key]
    ):
      symms, _, _ = self._split_and_pad_obs(symm_key, dones_key)
    ...
    for obs_group in obs_groups:
      obs_group_no_next = obs_group.replace('_next', '')
      is_next = obs_group.endswith('_next')
      hs_key = None
      if obs_group_no_next in hidden_states_keys:
        hs_key = f'{obs_group_no_next}_hidden_states'
      obs[obs_group], traj_masks, hidden_states[obs_group] = self._split_and_pad_obs(
        obs_group, dones_key, hs_key, is_next
      )
      if obs_group_no_next in obs_sym_groups:
        obs_sym[obs_group] = {}
        for key, value in obs[obs_group].items():
          if key in symms:
            obs_sym[obs_group][key] = symms[key]
          else:
            obs_sym[obs_group][key] = symmetry_fn(obs_group_no_next, key)(
              value
            ).detach()
    ...
    for _ in range(num_learning_epochs):
      ...
      with torch.no_grad():
        if last_value_items is not None:
          ...
          last_value = (
            list(last_value_network(*last_value_args)[0].values())[0].pred().detach()
          )
      for i in range(num_mini_batches):
        ...
        if first_traj == last_traj:
          utils.print(
            f'Skipping empty mini batch start: {start}, stop: {stop}, first_traj: {first_traj}, last_traj: {last_traj}',
            color='yellow',
          )
        ...
        for obs_group in obs_groups:
          obs_batch[obs_group] = pytree.tree_map(
            lambda x: x[:, first_traj:last_traj], obs[obs_group]
          )
          obs_group_no_next = obs_group.replace('_next', '')
          if obs_group_no_next in obs_sym_groups:
            obs_sym_batch[obs_group] = pytree.tree_map(
              lambda x: x[:, first_traj:last_traj], obs_sym[obs_group]
            )
          if obs_group_no_next in hidden_states_keys:
            hs = pytree.tree_map(
              lambda x: x[:, first_traj:last_traj], hidden_states[obs_group]
            )
        ...
        rl_values_batch = {}
        if last_value_items is not None:
          rl_values_batch['last_value'] = pytree.tree_map(
            lambda x: x[start:stop], last_value
          )
        ...
        with torch.no_grad():
          ...
          for key, network in networks.items():
            assert key in obs_groups, (
              f'Network key [{key}] not in obs_groups: [{obs_groups}]'
            )
            network_batch[key], _, _ = network(
              obs_batch[key], masks=masks_batch, hidden_states=hidden_states_batch[key]
            )
          for key, network in networks_sym.items():
            assert key in obs_sym_groups, (
              f'Network key [{key}] not in obs_sym_groups: [{obs_sym_groups}]'
            )
