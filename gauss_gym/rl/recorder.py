...
from gauss_gym.utils import config, timer, when
...
class Recorder:
  def __init__(
    ...
    self.fps = int(1.0 / (self.cfg['control']['decimation'] * self.cfg['sim']['dt']))
  ...
  def setup_recorder(self, env):
    ...
    for mesh_id in range(env.scene_manager.num_meshes):
      ...
      env.gym.set_camera_location(
        camera_handle, env.envs[env_id], gymapi.Vec3(*pos), gymapi.Vec3(*lookat)
      )
  ...
  def update_frames(self, env, image_features={}):
    ...
    for mesh_name, env_handle, camera_handle, env_id in zip(
      self.mesh_names, self.env_handles, self.camera_handles, self.env_ids
    ):
      image = env.gym.get_camera_image(
        env.sim, env_handle, camera_handle, gymapi.IMAGE_COLOR
      )
  ...
  def maybe_init(self):
    ...
    if self.rank_zero:
      ...
      self.writer = tf.summary.create_file_writer(
        str(self.log_dir / 'summaries'), max_queue=int(1e9), flush_millis=int(1e9)
      )
      self.writer.set_as_default()
      if self.cfg['runner']['use_wandb']:
        run_description = self.cfg['runner']['description']
        if self.run_desc is not None:
          run_description += self.run_desc
        wandb.init(
      ...
      if (
        self.cfg['runner']['use_wandb']
        and self.cfg['runner']['wandb_save_model']
        and self.rank_zero
      ):
        wandb.save(self.log_dir / 'train_config.yaml', base_path=self.log_dir)
      config.Config({'deploy': self.deploy_cfg}).save(
        self.log_dir / 'deploy_config.yaml'
      )
  ...
  def record_episode_statistics(
    self, done, ep_info, it, discount_factor_dict={}, write_record=False
  ):
    self.maybe_init()
    ...
    if self.rank_zero:
      for key, value in statistics.items():
        if isinstance(value, str):
          ...
          isinstance(value, np.ndarray) and value.ndim == 4 and value.dtype == np.uint8
        ):
          ...
          if self.cfg['runner']['use_wandb']:
            ...
            wandb.log({key: wandb.Video(value, fps=self.fps)}, step=it)
        else:
