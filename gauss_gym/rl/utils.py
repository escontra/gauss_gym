...
def init_linear(
  ...
  if dist == 'trunc_normal':
    torch.nn.init.trunc_normal_(layer.weight, a=-2.0, b=2.0)
    ...
    layer.weight.data *= 1 / torch.linalg.norm(
      layer.weight.data.reshape((-1, layer.weight.data.shape[-1])), 2, 0
    )
  else:
...
def broadcast_moments(tensor: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
  ...
  local_count = torch.tensor(
    [tensor_flat.numel()], device=tensor.device, dtype=torch.long
  )
...
def broadcast_object(object: Union[List, Any], src_rank: int):
  """
  Broadcasts an object only within the local node (no network communication).

  Args:
      object: The object to broadcast
      src_rank: Source rank for the broadcast within the local node
  """
  ...
  # Use broadcast_object_list which will only communicate within the local process group
  torch_distributed.broadcast_object_list(object_list, src_rank)
...
def broadcast_left(y: torch.Tensor, x: torch.Tensor):
  "Broadcast y to the same shape as x by broadcasting left."
  num_expand = len(x.shape) - len(y.shape)
  y_expanded = y.clone()
  for _ in range(num_expand):
    y_expanded = y_expanded.unsqueeze(0)
  y_expanded = y_expanded.expand_as(x).detach()
  return y_expanded
...
def mirror_latent(latent):
  return pytree.tree_map(lambda x: -1.0 * x, latent)


def split_and_pad_trajectories(tensor, dones):
  """Splits trajectories at done indices. Then concatenates them and pads
  with zeros up to the length of the longest trajectory.
  ...
  if padded_trajectories.shape[0] < tensor.shape[0]:
    padding = torch.zeros(
      tensor.shape[0] - padded_trajectories.shape[0],
...
def unpad_trajectories(trajectories, masks):
  ...
  traj_viewed = traj_indexed.view(
    -1, trajectories.shape[0], *trajectories.shape[-obs_num_dims:]
  )
  traj_viewed_transp = traj_viewed.transpose(1, 0)
  return traj_viewed_transp


def get_mlp_cnn_keys(
  obs, project_dims={}
) -> Tuple[Optional[List[str]], Optional[List[str]], Optional[List[str]], int]:
