...
from gauss_gym.utils import space


class RunningEMA:
  def __init__(self, alpha=0.9):
    self.running_ema = 0.0
    self.alpha = alpha
...
def _validate_batch_shapes(
  batch, reference_sample, batch_dims: Tuple[int, ...]
) -> None:
  ...
  def validate_node_shape(reference_sample: torch.Tensor, batch: torch.Tensor) -> None:
...
class PyTreeNormalizer(nn.Module):
  def __init__(
    ...
    self.count = nn.Parameter(
      torch.zeros(size=(), dtype=torch.int32), requires_grad=False
    )
  ...
  def update(self, obs_tree, validate_shapes: bool = True):
    ...
    batch_dims = batch_shape[
      : len(batch_shape) - pytree.tree_leaves(self.mean.to_dict())[0].ndim
    ]
    ...
    def _compute_node_statistics(
      mean: torch.Tensor, summed_variance: torch.Tensor, batch: torch.Tensor
    ...
    updated_stats = pytree.tree_map(
      _compute_node_statistics,
      self.mean.to_dict(),
      self.summed_variance.to_dict(),
      obs_tree,
    )
    # Extract `mean` and `summed_variance` from `updated_stats` nest.
    self.mean.update(
      pytree.tree_map(lambda _, x: x[0], self.mean.to_dict(), updated_stats)
    )
    self.summed_variance.update(
      pytree.tree_map(lambda _, x: x[1], self.mean.to_dict(), updated_stats)
    )

    def compute_std(summed_variance: torch.Tensor, std: torch.Tensor) -> torch.Tensor:
    ...
    self.std.update(
      pytree.tree_map(compute_std, self.summed_variance.to_dict(), self.std.to_dict())
    )
  ...
  def denormalize(self, obs_tree):
    ...
    return pytree.tree_map(
      lambda data, mean, std: denormalize_leaf(
        data, mean, std, use_mean_offset=self.use_mean_offset
      ),
      obs_tree,
      self.mean.to_dict(),
      self.std.to_dict(),
...
class DictNormalizer(nn.Module):
  def __init__(
    ...
    self.dont_normalize_keys = (
      dont_normalize_keys if dont_normalize_keys is not None else []
    )
    ...
    for k, v in obs_space.items():
      ...
      self.register_buffer(
        f'summed_variance_{k}', torch.zeros(v.shape, dtype=torch.float32)
      )
...
class RewardNormalizer(nn.Module):
  def __init__(
    self, gamma: float, num_envs: int, g_max: float = 10.0, epsilon: float = 1e-8
  ):
    super().__init__()
    self.G = 0.0
    self.G_rms = DictNormalizer(obs_space={'G': space.Space(torch.float32, ())})
  ...
  def update(self, rewards: torch.Tensor, dones: torch.Tensor):
    ...
    self.G_r_max.data = torch.maximum(self.G_r_max.data, torch.max(torch.abs(self.G)))


class BackwardReturnTracker(nn.Module):
  def __init__(self, obs_tree, discount: float):
    super().__init__()
  ...
  def __call__(self, rew_tree, is_first, validate_shapes: bool = True):
    assert pytree.tree_structure(rew_tree) == pytree.tree_structure(
      self.bwreturn.to_dict()
    )
    ...
    # We assume the batch dimensions always go first.
    batch_dims = batch_shape[
      : len(batch_shape) - pytree.tree_leaves(self.bwreturn.to_dict())[0].ndim
    ]
    ...
    def _compute_node_statistics(
      bwreturn: torch.Tensor, batch: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
      # The mean and the sum of past variances are updated with Welford's
      # algorithm using batches (see https://stackoverflow.com/q/56402955).
    ...
    self.bwreturn.update(
      pytree.tree_map(_compute_node_statistics, self.bwreturn.to_dict(), rew_tree)
    )
