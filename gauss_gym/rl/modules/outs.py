...
from gauss_gym.rl.modules import decoder
...
class Normal:
  ...
  def kl(self, other: 'Normal') -> torch.Tensor:
    ...
    return 0.5 * (
      ...
      + 2 * torch.log(other.stddev)
      - 2 * torch.log(self.stddev)
      - 1
    )
...
class TwoHot:
  ...
  def loss(self, target):
    ...
    above = len(self.bins) - (self.bins > target[..., None]).to(torch.int32).sum(-1)
...
class NormalLogSTDHead(nn.Module):
  def __init__(
    ...
    if bounded:
      mean_net = nn.Sequential(mean_net, nn.Tanh())
    self.mean_net = mean_net
    self.logstd = torch.nn.parameter.Parameter(
      torch.full(
        (1, int(np.prod(self.output_size))),
        fill_value=math_utils.std_to_logstd(
          torch.tensor(self.init_std), self.minstd, self.maxstd
        ),
        dtype=torch.float32,
      ),
      requires_grad=True,
  ...
  def stats(self) -> Dict[str, torch.Tensor]:
    return {
      ...
      'std': math_utils.logstd_to_std(self.logstd, self.minstd, self.maxstd).detach(),
    }

  def forward(self, x: torch.Tensor) -> Normal:
    mean = self.mean_net(x)
    std = math_utils.logstd_to_std(self.logstd, self.minstd, self.maxstd)
...
class NormalLogSTDHeadStateDependent(nn.Module):
  def __init__(
    self,
    input_size: int,
    output_space: space.Space,
    ...
    self.maxstd = maxstd
    mean_net = nn.Linear(input_size, int(np.prod(self.output_size)))
    logstd_net = nn.Linear(input_size, int(np.prod(self.output_size)))
    if outscale is not None:
      utils.init_linear(mean_net, scale=outscale)
      utils.init_linear(logstd_net, scale=outscale)
    if bounded:
      mean_net = nn.Sequential(mean_net, nn.Tanh())
    self.mean_net = mean_net
    self.logstd_net = logstd_net
  ...
  def forward(self, x: torch.Tensor) -> Normal:
    ...
    std = math_utils.logstd_to_std(logstd, self.minstd, self.maxstd)
    mean = utils.reshape_output(mean, self.output_size)
    std = utils.reshape_output(std, self.output_size)
    output = Normal(mean, std)
    return output
...
class TwoHotHead(nn.Module):
  def __init__(
    ...
    self.projection_net = nn.Linear(
      input_size, int(np.prod(self.output_size) * self.num_bins)
    )  # , **self.kw)
  ...
  def forward(self, x: torch.Tensor) -> TwoHot:
    if self.num_bins % 2 == 1:
      half = torch.linspace(
        -20, 0, (self.num_bins - 1) // 2 + 1, dtype=torch.float32, device=x.device
      )
      half = math_utils.symexp(half)
      bins = torch.cat([half, -half[:-1].flip(0)], dim=0)
    else:
      half = torch.linspace(
        -20, 0, self.num_bins // 2, dtype=torch.float32, device=x.device
      )
      half = math_utils.symexp(half)
      bins = torch.cat([half, -half.flip(0)], dim=0)

    logits = self.projection_net(x)
    logits = utils.reshape_output(
      logits, (int(np.prod(self.output_size)), self.num_bins)
    )
...
class VoxelGridDecoderHead(nn.Module):
  def __init__(
    ...
    self.occupancy_grid_projection = nn.Conv3d(
      in_channels=input_size, out_channels=1, kernel_size=1, stride=1
    )
    self.centroid_grid_projection = nn.Sequential(
      nn.Conv3d(in_channels=input_size, out_channels=1, kernel_size=1, stride=1),
      nn.Sigmoid(),
    )
