...
from gauss_gym.utils import math_utils, space
from gauss_gym.rl import utils as rl_utils
from gauss_gym.rl.modules import outs, normalizers
from gauss_gym import GAUSS_GYM_ROOT_DIR
...
class RecurrentModel(abc.ABC):
  ...
  @abc.abstractmethod
  def reset(
    self, dones: torch.Tensor, hidden_states: Optional[Tuple[torch.Tensor, ...]] = None
  ) -> Tuple[torch.Tensor, ...]:
    pass
  ...
  def forward(
    ...
    Dict[str, Union[outs.Output, torch.Tensor]], Optional[Tuple[torch.Tensor, ...]]
  ]:
    pass
...
class RecurrentCNNModel(torch.nn.Module, RecurrentModel):
  def __init__(
    ...
    self.recurrent_model = RecurrentMLPModel(
      ...
      dont_normalize_keys=dont_normalize_keys
      + list(self.image_feature_model.obs_space().keys()),
      head=head,
  ...
  @torch.jit.export
  def reset(
    self, dones: torch.Tensor, hidden_states: Optional[_RNN_STATE] = None
  ) -> _RNN_STATE:
  ...
  def rnn_parameters(self):
    return list(self.recurrent_model.memory.parameters()) + list(
      self.recurrent_model.rnn_proj.parameters()
    )

  @torch.jit.ignore
  def decoder_parameters(self):
    return list(self.recurrent_model.model.parameters()) + list(
      self.recurrent_model.heads.parameters()
    )
  ...
  def forward(
    ...
    unpad: bool = True,
    rnn_only: bool = False,
  ) -> Tuple[Optional[Dict[str, _DIST]], torch.Tensor, Optional[_RNN_STATE]]:
    ...
    for k, v in image_feature_obs.items():
      new_obs[k] = v
    return self.recurrent_model.forward(new_obs, hidden_states, masks, unpad, rnn_only)
...
class ImageFeature(torch.nn.Module):
    ...
    if self.cnn_keys is not None:
      for key in self.cnn_keys:
        ...
        if self.model_type == 'cnn_simple':
          from gauss_gym.rl.modules import resnet
          ...
          from gauss_gym.rl.modules.dino import backbones

          dino = getattr(backbones, self.model_type)(pretrained=dino_pretrained)
          if hasattr(dino, 'patch_size'):
            dino.register_forward_pre_hook(
              lambda module, inputs: (
                backbones.CenterPadding(dino.patch_size)(inputs[0]).clone(),
              )
            )
          ...
          if from_local:
            model_name = (
              f'local-dir:{GAUSS_GYM_ROOT_DIR}/resources/encoders/{model_name}'
            )
            utils.print(
              f'Loading local ImageFeature encoder: {model_name}', color='blue'
            )
          timm_model = timm.create_model(
            model_name, pretrained=dino_pretrained, num_classes=0
          )
  ...
  def forward(self, obs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    ...
    for key, encoder in self.encoder.items():
      ...
      cnn_obs = rl_utils.flatten_batch(
        cnn_obs, spatial_dims
      )  # Shape: [M*N*L*O, C, H, W]
  ...
  def modified_obs_space(self) -> Dict[str, space.Space]:
    ...
    modified_obs_space = {}
    for key in self.cnn_keys:
      modified_obs_space[key] = space.Space(
        torch.float32, (self.embedding_dim,), -torch.inf, torch.inf
      )
...
class RecurrentMLPModel(torch.nn.Module, RecurrentModel):
  def __init__(
    ...
    for k, v in project_dims.items():
      input_projectors[k] = nn.Sequential(
        ...
        rl_utils.get_activation(layer_activation),
      )
    if input_projectors:
    ...
    self.mlp_keys, mlp_2d_reshape_keys, self.cnn_keys, self.num_mlp_obs = (
      rl_utils.get_mlp_cnn_keys(obs_space, project_dims)
    )
    self.mlp_2d_reshape_keys: List[str] = mlp_2d_reshape_keys or ['_DEFAULT_UNUSED']
    assert self.cnn_keys is None or len(self.cnn_keys) == 0, (
      f'CNN keys are not supported for {self.__class__.__name__}, got: [{self.cnn_keys}]'
    )
    ...
    self.memory = Memory(
      self.obs_size, type='lstm', num_layers=1, hidden_size=self.recurrent_state_size
    )
    self.normalize_obs = normalize_obs
    self.max_abs_value = max_abs_value
    print(f'{self.__class__.__name__}:')
    if self.normalize_obs:
      self.obs_normalizer = normalizers.DictNormalizer(
        obs_space,
    ...
    if self.recurrent_proj_size > 0:
      self.rnn_proj = torch.nn.Linear(
        self.recurrent_state_size, self.recurrent_proj_size
      )
    else:
    ...
    for size in hidden_layer_sizes:
      ...
      layers.append(rl_utils.get_activation(layer_activation))
      input_size = size
    output_size = input_size
    self.model = torch.nn.Sequential(*layers)
    heads = {}
    for k, v in action_space.items():
      heads[k] = getattr(outs, head[k]['class_name'])(
        output_size, v, **head[k]['params']
      )
    ...
    for k in self.mlp_keys:
      ...
      if self.normalize_obs and k not in self.dont_normalize_keys:
        ...
        if self.max_abs_value is not None:
          processed_obs[k] = torch.clamp(
            processed_obs[k], -self.max_abs_value, self.max_abs_value
          )
      if k in self.mlp_2d_reshape_keys:
        processed_obs[k] = rl_utils.flatten_obs(
          processed_obs[k], self.obs_space[k].shape
        )
      if self.symlog_inputs:
        processed_obs[k] = math_utils.symlog(processed_obs[k])
  ...
  def forward(
    ...
    # RNN.
    rnn_state, new_hidden_states = self.memory(
      processed_obs, hidden_states, masks, unpad=unpad
    )
  ...
  def rnn_state_size(self):
    rnn_size = (
      self.recurrent_proj_size
      if self.recurrent_proj_size > 0
      else self.recurrent_state_size
    )
    if self.recurrent_skip_connection:
...
class Memory(torch.nn.Module):
  def __init__(
    ...
    self.initial_hidden_state = nn.Parameter(
      torch.zeros(self.num_layers, 1, self.hidden_size), requires_grad=True
    )
  ...
  def forward(
    ...
    masks: Optional[torch.Tensor] = None,
    unpad: bool = True,
  ) -> Tuple[torch.Tensor, Optional[_RNN_STATE]]:
  ...
  def reset(
    ...
    if self.is_lstm:
      if hidden_states is not None:
        hidden_states[0][:, dones, :] = (
          self.initial_hidden_state.detach().clone().to(hidden_states[0].device)
        )
        hidden_states[1][:, dones, :] = (
          self.initial_cell_state.detach().clone().to(hidden_states[1].device)
        )
