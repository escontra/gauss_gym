import dataclasses
from gauss_gym.utils import observations as O
...
def observation_groups_from_config(config: Dict) -> List[ObservationGroup]:
  ...
  for name, cfg in config.items():
    ...
    if 'sync_latency' in cfg:
      ...
      assert all(
        obs.latency_range == sync_latency[0].latency_range for obs in sync_latency
...
DOF_FRICTION = Observation(
  name='dof_friction',
  func=O.dof_friction,
)

DOF_ARMATURE = Observation(
  name='dof_armature',
  func=O.dof_armature,
)
