...
class BatchWireframeSphereGeometry(gymutil.LineGeometry):
  ...
  def draw(
    self, positions, gym, viewer, env, only_render_selected=-1, custom_colors=None
  ):
    flat_pos = (
      positions.repeat_interleave(self.num_lines, dim=0).view(-1, 3).cpu().numpy()
    )
    if custom_colors is not None:
      colors_tmp = np.empty(self.colors().shape[0], gymapi.Vec3.dtype)
      custom_colors = (
        custom_colors.repeat_interleave(self.num_lines, dim=0).view(-1, 3).cpu().numpy()
      )
    ...
    verts_tmp = self.verts_tmp[
      start_frustrum * self.num_lines : end_frustrum * self.num_lines
    ]
    colors_tmp = colors_tmp[
      start_frustrum * self.num_lines : end_frustrum * self.num_lines
    ]
...
class BatchWireframeAxisGeometry(gymutil.LineGeometry):
  ...
  def draw(
    ...
    if axis_scales is not None:
      ...
      axis_scales = torch.stack(
        (axis_scales_x, axis_scales_y, axis_scales_z), dim=1
      ).reshape(-1, 3)
      axis_scales = axis_scales.repeat_interleave(self.lines_per_axis, dim=0)
    verts_stacked = np.stack(
      (self.verts['x'], self.verts['y'], self.verts['z']), axis=-1
    )
    if axis_scales is not None:
...
class BatchWireframeFrustumGeometry(gymutil.LineGeometry):
  ...
  def __init__(
    ...
    edges = [
      ...
      (v1, v2),
      (v2, v3),
      (v3, v0),  # Primary edges clockwise
      (v2, v1),
      (v0, v3),  # Redundant bottom and top edges
      # Far plane
      (v4, v5),
      (v5, v6),
      (v6, v7),
      (v7, v4),  # Primary edges clockwise
      (v6, v5),
      (v4, v7),  # Redundant bottom and top edges
      # Connecting edges
      (v0, v4),
      (v1, v5),
      (v2, v6),
      (v3, v7),  # Connect near & far
    ]
  ...
  def draw(self, positions, rotations, gym, viewer, env, only_render_selected=-1):
    flat_pos = positions.repeat_interleave(self.num_lines, dim=0).cpu().numpy()
    flat_quat = (
      rotations.repeat_interleave(self.num_lines, dim=0).cpu().numpy()
    )  # (N, 4)
    ...
    if only_render_selected >= 0:
      verts_tmp = self.verts_tmp[
        only_render_selected * self.num_lines : (only_render_selected + 1)
        * self.num_lines
      ]
      colors_tmp = self._colors[
        only_render_selected * self.num_lines : (only_render_selected + 1)
