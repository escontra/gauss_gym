...
from gauss_gym.utils import observation_groups, math_utils, timer, space
    ...
    for obs_group in obs_groups_cfg:
      ...
      for obs in obs_group.observations:
        ...
        if obs.ignore_in_observation_manager:
          continue
        latency_range = obs.latency_range if obs_group.add_latency else (0.0, 0.0)
        function = obs.func
        ...
        self.obs_dims_per_group_obs[obs_group.name][obs.name] = space.Space(
          shape=obs_shape[1:], dtype=obs_dtype
        )
        ...
        self.obs_buffers_per_group[obs_group.name][obs.name] = torch.zeros(
          (buffer_length, env.num_envs, *obs_shape[1:]),
          dtype=obs_dtype,
          device=self.device,
        )
        if obs.name in sync_latency_names:
          if sync_latency_buffer is None:
            sync_latency_buffer = self._sample_latency_buffer(
              latency_range, env.num_envs
            )
          ...
          self.latency_buffers_per_group[obs_group.name][obs.name] = (
            self._sample_latency_buffer(latency_range, env.num_envs)
          )
        if obs.noise_range:
          self.noise_level_per_group[obs_group.name][obs.name] = (
            math_utils.sample_uniform(*obs.noise_range, (env.num_envs,), self.device)
          )
        ...
        self.obs_available_for_timestep[obs_group.name][obs.name] = torch.zeros(
          (buffer_length, env.num_envs), dtype=torch.bool, device=self.device
        )

  def _sample_latency_buffer(self, latency_range, size):
    return math_utils.torch_rand_float(
      *latency_range,
      (size, 1),
      device=self.device,
    ).flatten()
  ...
  def resample_sensor_latency(self, env_ids=None):
    for obs_group in self.obs_group_cfg:
      ...
      if env_ids is None:
        ...
        env_ids = (
          (self.env.episode_length_buf % _int == 0).nonzero(as_tuple=False).flatten()
        )
      if len(env_ids) == 0:
        continue
      sync_latency_names, sync_latency_buffer = [], None
      ...
      for obs in obs_group.observations:
        ...
        if obs.name in sync_latency_names:
          ...
          self.latency_buffers_per_group[obs_group.name][obs.name][env_ids] = (
            sync_latency_buffer
          )
        else:
          self.latency_buffers_per_group[obs_group.name][obs.name][env_ids] = (
            self._sample_latency_buffer(latency_range, len(env_ids))
          )
  ...
  def reset_buffers(self, env_ids):
    ...
    for obs_group in self.obs_group_cfg:
      for obs in obs_group.observations:
        ...
        self.delayed_frames_per_group[obs_group.name][obs.name][env_ids] = 1
        self.obs_available_for_timestep[obs_group.name][obs.name][:, env_ids] = False
        if obs.noise_range:
          self.noise_level_per_group[obs_group.name][obs.name][env_ids] = (
            math_utils.sample_uniform(*obs.noise_range, (len(env_ids),), self.device)
          )
    self.initialize_env_ids = env_ids

  @timer.section('compute_obs')
  def compute_obs(self, env, update_buffer_mask: Optional[torch.Tensor] = None):
    if update_buffer_mask is None:
      update_buffer_mask = torch.ones(
        self.env.num_envs, device=self.device, dtype=torch.bool
      )
    obs_dict = {}
    for obs_group in self.obs_group_cfg:
      obs_dict[obs_group.name] = {}
      for obs in obs_group.observations:
        if obs.ignore_in_observation_manager:
          continue
        ...
        if len(self.initialize_env_ids) > 0 and new_obs is not None:
          ...
          self.obs_buffers_per_group[obs_group.name][obs.name][
            :, self.initialize_env_ids
          ] = fill_obs
          self.obs_available_for_timestep[obs_group.name][obs.name][
            :, self.initialize_env_ids
          ] = True
        ...
        self.obs_buffers_per_group[obs_group.name][obs.name][:, update_buffer_mask] = (
          torch.roll(
            self.obs_buffers_per_group[obs_group.name][obs.name][:, update_buffer_mask],
            -1,
            dims=0,
        ...
        if new_obs is not None:
          self.obs_buffers_per_group[obs_group.name][obs.name][
            -1, update_buffer_mask
          ] = new_obs[update_buffer_mask]
          self.obs_available_for_timestep[obs_group.name][obs.name][
            -1, update_buffer_mask
          ] = True
        else:
          self.obs_buffers_per_group[obs_group.name][obs.name][
            ...
            self.obs_buffers_per_group[obs_group.name][obs.name][-1, update_buffer_mask]
          )
          self.obs_available_for_timestep[obs_group.name][obs.name][
            -1, update_buffer_mask
          ] = False
        ...
        if int(obs.refresh_duration / self.env.dt) == 0:
          delay_refresh_mask = torch.ones_like(
            self.env.episode_length_buf, device=self.device, dtype=torch.bool
          )
        else:
          delay_refresh_mask = (
            self.env.episode_length_buf % int(obs.refresh_duration / self.env.dt)
          ) == 0

        # Get the delayed frames.
        buffer_delayed_frames = (
          (self.latency_buffers_per_group[obs_group.name][obs.name] / self.env.dt) + 1
        ).to(int)

        self.delayed_frames_per_group[obs_group.name][obs.name][update_buffer_mask] = (
          torch.where(
            delay_refresh_mask[update_buffer_mask],
            buffer_delayed_frames[update_buffer_mask],
            self.delayed_frames_per_group[obs_group.name][obs.name][update_buffer_mask]
            + 1,
          )
        )
        ...
        if obs_group.add_noise and obs.noise_range:
          obs_value = self._add_uniform_noise(
            obs_value, self.noise_level_per_group[obs_group.name][obs.name]
          )
        if obs.clip:
        ...
        if obs.scale is not None:
          ...
          obs_value *= scale
        obs_dict[obs_group.name][obs.name] = obs_value
    self.initialize_env_ids = []
    return obs_dict
