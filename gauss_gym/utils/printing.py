...
REGEX_TOKEN = re.compile(r"([^a-zA-Z0-9-_./'\"\[\]]|['\"][^\s]*['\"])", re.MULTILINE)
REGEX_NUMBER = re.compile(r'([-+]?[0-9]+[0-9.,]*(e[-+]?[0-9])?|nan|-?inf)')
KEYWORDS = (
  'True',
  'False',
  'None',
  ...
  if isinstance(color, str) or bold:
    ...
    for i, token in enumerate(tokens[:-1]):
      ...
      if new[3]:  # Highlighted
        ...
        new[2] = False
      if new != prev:
        result.append(style(color=new[0], bold=new[1], underline=new[2], reset=True))
      result.append(token)
      prev = new
      if '\n' in token:
        prev[1] = None
