...
from huggingface_hub import hf_hub_download, list_repo_files, repo_exists, errors
...
class HFPath(Path):
  __slots__ = ('_path', '_repo_id', '_repo_type', '_filename')
  ...
  def _parse_path(self):
    path = str(self)[5:]  # Remove 'hf://'
    if '/' in path:
      parts = path.split('/', 2)
      self._repo_id = f'{parts[0]}/{parts[1]}'
      self._filename = parts[2] if len(parts) > 2 else None
    else:
      self._repo_id = path
      self._filename = None
  ...
  def open(
    self, mode='r', wait_for_rank_zero_open=False, wait_for_rank_zero_timeout=120.0
  ):
    assert self._filename, 'is a directory'
    if 'r' in mode:
      if wait_for_rank_zero_open:
        filepath = None
        start = time.time()
        while filepath is None:
          try:
            ...
            utils.print(
              f'Waiting for {self._filename} to become available: {e}', color='yellow'
            )
            time.sleep(1)
          if time.time() - start > wait_for_rank_zero_timeout:
            raise RuntimeError(
              f'{self._filename} did not become available within {wait_for_rank_zero_timeout} seconds.'
            )
      else:
        filepath = hf_hub_download(
          self._repo_id, self._filename, repo_type=self._repo_type
        )
  ...
  def read(self, mode='r'):
    assert self._filename, 'is a directory'
    filepath = hf_hub_download(self._repo_id, self._filename, repo_type=self._repo_type)
    print(filepath)
  ...
  def glob(self, pattern):
    files = list_repo_files(self._repo_id, repo_type=self._repo_type)
    prefix = self._filename + '/' if self._filename else ''
    ...
    if pattern == '*':
      ...
      # Recursive glob
      pattern_full = prefix + pattern
      results = fnmatch.filter(files, pattern_full)
    else:
      # Simple glob
      pattern_full = prefix + pattern
      results = fnmatch.filter(files, pattern_full)

    return [type(self)(f'hf://{self._repo_id}/{x}') for x in results]

  def exists(self):
    if not repo_exists(self._repo_id, repo_type=self._repo_type):
      return False
    if not self._filename:
      return True
    files = list_repo_files(self._repo_id, repo_type=self._repo_type)
    return self._filename in files or any(
      f.startswith(self._filename + '/') for f in files
    )
  ...
  def isfile(self):
    if not self._filename:
      return False
    files = list_repo_files(self._repo_id, repo_type=self._repo_type)
    return self._filename in files

  def isdir(self):
    if not self._filename:
      return repo_exists(self._repo_id, repo_type=self._repo_type)
    files = list_repo_files(self._repo_id, repo_type=self._repo_type)
    return any(f.startswith(self._filename + '/') for f in files)

  def mkdir(self, **kwargs):
    raise NotImplementedError('Directory creation not supported for HF datasets')

  def remove(self, recursive=False):
    raise NotImplementedError('File removal not supported for HF datasets')

  def copy(self, dest, recursive=False):
    raise NotImplementedError('Copy operations not supported for HF datasets')

  def move(self, dest, recursive=False):
    raise NotImplementedError('Move operations not supported for HF datasets')

  @property
  def size(self):
    raise NotImplementedError('Size property not available for HF datasets')
      ...
      with GCS_LOCK:
        if not GCS_CLIENT:
          ...
          adapter = requests.adapters.HTTPAdapter(
            pool_connections=64, pool_maxsize=64, max_retries=3, pool_block=True
          )
