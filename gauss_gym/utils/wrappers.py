...
from gauss_gym.utils import space, observation_groups, math_utils
from gauss_gym.envs.base import legged_robot
from gauss_gym.rl import experience_buffer
...
class ImageEncoderWrapper(Wrapper):
  def __init__(
    self,
    env: legged_robot.LeggedRobot,
    image_encoder_key: str,
    ...
    self.image_encoder_space = space.Space(
      torch.float32, (self.image_encoder.rnn_state_size,), -torch.inf, torch.inf
    )
    ...
    self.noise_level = math_utils.sample_uniform(
  ...
  def obs_space(self):
    ...
    for obs_group in self.env.obs_groups:
      if observation_groups.IMAGE_ENCODER_LATENT in obs_group.observations:
        new_obs_space[obs_group.name][observation_groups.IMAGE_ENCODER_LATENT.name] = (
          self.image_encoder_space
        )
    return new_obs_space

  def add_latent_to_obs_dict(
    self,
    obs_dict: Dict[str, Dict[str, torch.Tensor]],
    final_obs_dict: Dict[str, Dict[str, torch.Tensor]],
    latent: torch.Tensor,
  ):
    for obs_group in self.env.obs_groups:
      if observation_groups.IMAGE_ENCODER_LATENT in obs_group.observations:
        ...
        final_obs_dict[obs_group.name][observation_groups.IMAGE_ENCODER_LATENT.name] = (
          latent
        )
  ...
  def reset(self):
    ...
    obs_dict, final_obs_dict = self.env.reset()
    self.noise_level[:] = math_utils.sample_uniform(
      *observation_groups.IMAGE_ENCODER_LATENT.noise_range,
      (self.env.num_envs,),
      self.device,
    ...
    with torch.no_grad():
      (
        self.env.image_encoder_dists,
        self.image_encoder_rnn_state,
        self.image_encoder_hidden_states,
      ) = self.image_encoder(
        obs_dict[self.image_encoder_key],
        self.image_encoder_hidden_states,
        rnn_only=self.env.headless,
      )
    return self.add_latent_to_obs_dict(
      obs_dict, final_obs_dict, self.image_encoder_rnn_state
    )

  def _check_observation_available(
    self, obs_dict, key: str, obs_space: Dict[str, Dict[str, space.Space]]
  ):
  ...
  def step(self, actions: Dict[str, torch.tensor], actions_mean=None):
    obs_dict, final_obs_dict, rew, done, time_out_buf, infos = self.env.step(
      actions, actions_mean
    )
    done_ids = done.nonzero(as_tuple=False).flatten()
    self.noise_level[done_ids] = math_utils.sample_uniform(
      *observation_groups.IMAGE_ENCODER_LATENT.noise_range,
      (len(done_ids),),
      self.device,
    )
    ...
    if self._check_observation_available(
      obs_dict, self.image_encoder_key, self.image_encoder.obs_space
    ):
      if self.buffer is not None:
        # Update the buffer.
        self.buffer.update_data(
      ...
      self.image_encoder_hidden_states = self.image_encoder.reset(
        self.dones, self.image_encoder_hidden_states
      )
      self.dones[:] = False
