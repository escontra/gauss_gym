...
def gather_concat(aggregator: Agg, world_size: int) -> Dict[str, np.ndarray]:
  local_concat = {}
  for k, v in aggregator.reducers.items():
    assert isinstance(v, Concat), 'Only Concat reducer is supported.'
    local_concat[k] = v.current()

  gathered_concat: List[Dict[str, np.ndarray]] = [None] * world_size  # type: ignore
  import torch.distributed as torch_distributed

  torch_distributed.all_gather_object(gathered_concat, local_concat)

  all_concat = {}
  for curr_concat_ in gathered_concat:
    for k, v in curr_concat_.items():
      if k not in all_concat:
        all_concat[k] = v
      else:
        all_concat[k] = np.concatenate([all_concat[k], v])
