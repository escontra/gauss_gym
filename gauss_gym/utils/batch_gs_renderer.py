...
class BatchPLYRenderer:
  def __init__(self, ply_path: path.Path, device: str = 'cuda'):
    """
    ...
    (
      ...
      self.opacities,
      self.sh_degree,
    ) = self._load_ply_gaussians(ply_path)
...
class MultiSceneRenderer:
  def __init__(
    self,
    ply_paths: List[Union[str, path.Path]],
    renderer_gpus: List[Union[int, str]],
    output_gpu: Union[int, str],
  ):
    """
    ...
    """
    self.renderer_gpus = [
      f'cuda:{gpu}' if isinstance(gpu, int) else gpu for gpu in renderer_gpus
    ]
    self.output_gpu = (
      f'cuda:{output_gpu}' if isinstance(output_gpu, int) else output_gpu
    )
    self.renderers = {}
    utils.print(
      f'Rendering on GPUs: {self.renderer_gpus}, Outputting to GPU: {self.output_gpu}',
      color='cyan',
    )
    ...
    for i, ply_path in enumerate(ply_paths):
      ...
      self.renderers[ply_path] = BatchPLYRenderer(ply_path, device=gpu_device)
  ...
  def batch_render(
    ...
    h: int,
    w: int,
    camera_linear_velocity: Union[
      Dict[str, Float[Tensor, 'num_cameras 3']], None
    ] = None,
    camera_angular_velocity: Union[
      Dict[str, Float[Tensor, 'num_cameras 3']], None
    ] = None,
    motion_blur_frac: float = 0.0,
    blur_samples: int = 5,
    ...
    str, Tuple[Float[Tensor, 'num_cameras h w 3'], Float[Tensor, 'num_cameras h w 1']]
  ]:
    """
    ...
    with torch.no_grad():
      ...
      for ply_path, c2ws in scene_poses.items():
        ...
        if ply_path not in self.renderers:
          raise KeyError(
            f'No renderer initialized for {ply_path}. Available renderers: {list(self.renderers.keys())}'
          )
        ...
        imgs, depths = renderer.batch_render(
          c2ws,
          curr_fl_x,
          curr_fl_y,
          curr_pp_x,
