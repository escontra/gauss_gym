...
path = pathlib.Path(GAUSS_GYM_ROOT_DIR) / 'resources' / 'encoders' / model_name
...
transfs = timm.data.create_transform(**data_config, is_training=False)
print(transfs)
...
out = model(transfs(img).unsqueeze(0).to('cuda'))
print(out.shape)
