...
from geometry_msgs.msg import Twist, Pose  # noqa: F401
...
class UnitreeA1Real:
  ...
  def __init__(
    ...
    self.policy_uses_vision = (
      observation_groups.IMAGE_ENCODER_LATENT in self.policy_obs_group.observations
    )
    if self.vision_only:
      assert self.vision_model is not None, (
        'vision_model is not provided in vision-only mode.'
      )
  ...
  def process_configs(self):
    ...
    self.observation_groups = observation_groups.observation_groups_from_config(
    ...
    if self.computer_clip_torque:
      rospy.loginfo(
        '[Env] torque limit: {:.1f} {:.1f} {:.1f}'.format(*self.torque_limits[:3])
      )
  ...
  def start_ros(self):
    ...
    if self.vision_only:
      self.image_subscriber = rospy.Subscriber(
        self.robot_namespace + self.image_topic, Image, self.update_image, queue_size=1
      )
  ...
  def compute_policy_observation(self):
    ...
    assert hasattr(self, 'legs_cmd_publisher') or self.vision_only, (
      'start_ros() not called, ROS handlers are not initialized!'
    )
    obs_dict = {}
    ...
    if self.policy_uses_vision and hasattr(self, 'visual_embedding_buffer'):
      obs_dict[self.vision_obs_group.name]['image_encoder'] = (
        self.visual_embedding_buffer
      )
    return obs_dict

  def compute_vision_observation(self):
    assert self.vision_only, (
      'compute_vision_observation() is only available in vision-only mode.'
    )
    obs_dict = {}
  ...
  def publish_legs_cmd(self, robot_coordinates_action, kp=None, kd=None):
    ...
    for sim_joint_idx in range(12):
      ...
      legs_cmd.cmd[real_joint_idx].q = (
        robot_coordinates_action[0, sim_joint_idx]
        if self.control_type == 'P'
        else rospy.get_param(self.robot_namespace + '/PosStopF', (2.146e9))
      )
      legs_cmd.cmd[real_joint_idx].dq = 0.0
      legs_cmd.cmd[real_joint_idx].tau = 0.0
      legs_cmd.cmd[real_joint_idx].Kp = (
        self.p_gains[sim_joint_idx] if kp is None else kp
      )
      legs_cmd.cmd[real_joint_idx].Kd = (
        self.d_gains[sim_joint_idx] if kd is None else kd
      )
  ...
  def update_gamepad(self, ros_msg):
    ...
    vel_x = (
      -1.0 * ros_msg.axes[1] * (lin_vel_range[1] - lin_vel_range[0]) + lin_vel_range[0]
    )
    vel_y = (
      -1.0 * ros_msg.axes[0] * (lin_vel_range[1] - lin_vel_range[0]) + lin_vel_range[0]
    )
    ang_vel = -1.0 * ros_msg.axes[2] * (ang_vel_range[1] - ang_vel_range[0]) / 2.0
  ...
  def update_image(self, ros_msg):
    ...
    rospy.loginfo_once(
      f'Image received: {image.shape}, {image.dtype} at {ros_msg.header.stamp}'
    )
    ...
    rospy.loginfo_once(
      f'Image downscaled: {resized_image.shape}, {resized_image.dtype}'
    )
  ...
  def publish_embedding(self, event):
    ...
    obs_dict[observation_groups.CAMERA_IMAGE.name] = np.transpse(
      self.image_buffer, (2, 0, 1)
    )[None]
