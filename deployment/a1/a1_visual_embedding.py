...
def get_started_pipeline(
  ...
  profile = pipeline.start(config)  # noqa: F841
...
def main(args):
  rospy.init_node('a1_gauss_gym_jetson')
  ...
  with open(osp.join(args.logdir, 'config.json'), 'r') as f:
    config_dict = json.load(f, object_pairs_hook=OrderedDict)
  if (
    config_dict.get('sensor', dict())
    .get('forward_camera', dict())
    .get('refresh_duration', None)
    is not None
  ):
  ...
  rospy.loginfo(
    'Depth range is clipped to [{}, {}] and normalized'.format(
  ...
  if args.enable_vis:
    rospy.loginfo(
      'Visualization enabled, sending depth{} images'.format(
        ', rgb' if args.enable_rgb else ''
      )
    )
  try:
    ...
    while not rospy.is_shutdown():
      ...
      frames = rs_pipeline.wait_for_frames(
        int(config_dict['sensor']['forward_camera']['latency_range'][1] * 1000)
      )  # ms
      ...
      if args.enable_rgb:
        ...
        if args.enable_vis and color_frame is not None:
      ...
      depth_image = (
        torch.from_numpy(depth_image_.astype(np.float32))
        .unsqueeze(0)
        .unsqueeze(0)
        .to(model_device)
      )
      ...
      if args.enable_vis:
        ...
        network_input_msg.header.frame_id = (
          args.namespace + '/camera_depth_optical_frame'
        )
