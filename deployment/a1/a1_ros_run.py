...
class StandOnlyModel(torch.nn.Module):
  def __init__(self, action_scale, dof_pos_scale, tolerance=0.2, delta=0.1):
    rospy.loginfo(
      'Using stand only model, please make sure the proprioception is 48 dim.'
    )
  ...
  def forward(self, obs):
    ...
    target_positions[diff_large_mask] = joint_positions[
      diff_large_mask
    ] - self.delta * torch.sign(joint_positions[diff_large_mask])
    return torch.clip(
...
def load_walk_policy(env, model_dir):
  ...
  if model_dir is None:
    ...
    privileged_obs_components = config_dict['env'].get(
      'privileged_obs_components', obs_components
    )
    ...
    model_action_scale = (
      torch.tensor(config_dict['control']['action_scale'])
      if isinstance(config_dict['control']['action_scale'], (tuple, list))
      else torch.tensor([config_dict['control']['action_scale']])[0]
    )
    if not (
      torch.is_tensor(model_action_scale)
      and (model_action_scale == env.action_scale).all()
    ):
    ...
    if (
      torch.is_tensor(action_rescale_ratio) and (action_rescale_ratio == 1.0).all()
    ) or (not torch.is_tensor(action_rescale_ratio) and action_rescale_ratio == 1.0):
      policy = policy_run
    else:
      policy = lambda x: policy_run(x) * action_rescale_ratio  # noqa: E731
...
def standup_procedure(
  ...
  while not rospy.is_shutdown():
    ...
    for i in range(12):
      target_pos[0, i] = (
        dof_pos[i] + direction[i] * angle_tolerance
        if abs(diff[i]) > angle_tolerance
        else env.default_dof_pos[i]
      )
    env.publish_legs_cmd(
      target_pos,
      kp=kp,
      kd=kd,
    )
  ...
  while not rospy.is_shutdown():
    if env.low_state_buffer.wirelessRemote.btn.components.R1:
      break
    if (
      env.low_state_buffer.wirelessRemote.btn.components.L2
      or env.low_state_buffer.wirelessRemote.btn.components.R2
    ):
...
class SkilledA1Real(UnitreeA1Real):
  ...
  def update_low_state(self, ros_msg):
    ...
    if (
      self.move_by_wireless_remote
      and ros_msg.wirelessRemote.ry > self.skill_mode_threhold
    ):
      skill_vel = (
        self.low_state_buffer.wirelessRemote.ry - self.skill_mode_threhold
      ) / (1.0 - self.skill_mode_threhold)
      skill_vel *= self.skill_vel_range[1] - self.skill_vel_range[0]
      skill_vel += self.skill_vel_range[0]
      self.command_buf[0, 0] = skill_vel
      self.command_buf[0, 1] = 0.0
      self.command_buf[0, 2] = 0.0
      return
...
def main(args):
  ...
  rospy.init_node('a1_gauss_gym_' + args.mode, log_level=log_level)
  ...
  unitree_real_env = SkilledA1Real(
    ...
    cfg=config_dict,
    forward_depth_topic='/visual_embedding'
    if args.mode == 'upboard'
    else '/camera/depth/image_rect_raw',
    forward_depth_embedding_dims=config_dict['policy']['visual_latent_size']
    if args.mode == 'upboard'
    else None,
  ...
  rospy.loginfo(
    'duration: {}, motor Kp: {}, motor Kd: {}'.format(
      duration,
  ...
  if args.mode == 'jetson':
    ...
    forward_depth_subscriber = rospy.Subscriber(  # noqa: F841
      args.namespace + '/camera/depth/image_rect_raw',
      Image,
      partial(
        handle_forward_depth,
    ...
    with torch.no_grad():
      ...
      while not rospy.is_shutdown():
        ...
        if unitree_real_env.low_state_buffer.wirelessRemote.btn.components.down:
          ...
          walk_model.reset()
        if (
          unitree_real_env.low_state_buffer.wirelessRemote.btn.components.L2
          or unitree_real_env.low_state_buffer.wirelessRemote.btn.components.R2
        ):
          unitree_real_env.publish_legs_cmd(
            unitree_real_env.default_dof_pos.unsqueeze(0), kp=2, kd=0.5
          )
          rospy.signal_shutdown('Controller send stop signal, exiting')
  elif args.mode == 'full':
    # extract and build the torch ScriptFunction
    ...
    with torch.no_grad():
      while not rospy.is_shutdown():
        ...
        motor_temperatures = [
          ...
          for motor_state in unitree_real_env.low_state_buffer.motorState
        ]
        rospy.loginfo_throttle(
          10,
          ' '.join(
            ['motor_temperatures:']
            + ['{:d},'.format(t) for t in motor_temperatures[:12]]
          ),
        )
        rate.sleep()
        if (
          ...
          unitree_real_env.publish_legs_cmd(
            unitree_real_env.default_dof_pos.unsqueeze(0), kp=20, kd=0.5
          )
          rospy.signal_shutdown('Controller send stop signal, exiting')
  else:
