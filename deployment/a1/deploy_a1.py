...
def main(argv=None):
  ...
  runner = deployment_runner_onnx.DeploymentRunner(cfg, model_name='policy')
  ...
  while not rospy.is_shutdown():
    ...
    if parsed.debug:
      with open(os.path.join(save_path, f'{rospy.Time.now()}.pkl'), 'wb') as f:
        pickle.dump(
          {'inputs': obs[cfg['policy']['obs_key']], 'actions': preds['actions']},
          f,
          protocol=5,
        )
    ...
    rospy.loginfo_throttle(
      10,
      ' '.join(
        ['motor_temperatures:'] + ['{:d},'.format(t) for t in motor_temperatures[:12]]
      ),
    )
    ...
    if unitree_real_env.quit_pressed:
      unitree_real_env.publish_legs_cmd(
        unitree_real_env.default_dof_pos[None], kp=20, kd=0.5
      )
      rospy.signal_shutdown('Controller send stop signal, exiting')
  return
