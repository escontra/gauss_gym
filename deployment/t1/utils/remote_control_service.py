...
class RemoteControlService:
  ...
  def get_operation_hint(self) -> str:
    if hasattr(self, 'joystick') and getattr(self, 'joystick') is not None:
      return (
        'Left axis for forward/backward/left/right, right axis for rotation left/right'
      )
    ...
    try:
      ...
      for device in devices:
        ...
        if evdev.ecodes.EV_ABS in caps and evdev.ecodes.EV_KEY in caps:
          ...
          if all(
            code in axes
            for code in [self.config.x_axis, self.config.y_axis, self.config.yaw_axis]
          ):
            absinfo = {}
  ...
  def _handle_axis(self, code: int, value: int):
    try:
      ...
      if code == self.config.x_axis:
        self.vx = self._scale(
          value,
          self.cmd_cfg['ranges']['lin_vel_x'][1],
          self.cmd_cfg['control_threshold'],
          code,
        )
        ...
        self.vy = self._scale(
          value,
          self.cmd_cfg['ranges']['lin_vel_y'][1],
          self.cmd_cfg['control_threshold'],
          code,
        )
        ...
        self.vyaw = self._scale(
          value,
          self.cmd_cfg['ranges']['ang_vel_yaw'][1],
          self.cmd_cfg['control_threshold'],
          code,
        )
  ...
  def close(self):
    ...
    if hasattr(self, 'joystick') and getattr(self, 'joystick') is not None:
      self.joystick.close()
    if (
      hasattr(self, 'joystick_runner') and getattr(self, 'joystick_runner') is not None
    ):
      self.joystick_runner.join(timeout=1.0)
    if (
      hasattr(self, 'keyboard_runner') and getattr(self, 'keyboard_runner') is not None
    ):
