...
assert 'MASA_PATH' in os.environ, 'MASA_PATH is not set'
MASA_PATH = os.environ['MASA_PATH']

# project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
sys.path.insert(0, MASA_PATH)
sys.path.insert(0, os.path.join(MASA_PATH, 'demo'))
...
def parse_args():
  ...
  parser.add_argument(
    '--save_masks_dir', type=str, default='masks', help='Directory to save masks'
  )
  parser.add_argument(
...
def main():
  args = parse_args()
  assert args.out, (
    'Please specify at least one operation (save the video) with the argument "--out" '
  )
  ...
  if args.unified:
    ...
    det_model = init_detector(
      args.det_config, args.det_checkpoint, palette='random', device=args.device
    )
    masa_model = init_masa(args.masa_config, args.masa_checkpoint, device=args.device)
    # build test pipeline
  ...
  visualizer = VISUALIZERS.build(masa_model.cfg.visualizer)  # noqa: F841

  if args.out:
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    video_writer = cv2.VideoWriter(
      args.out, fourcc, video_reader.fps, (video_reader.width, video_reader.height)
    )
  ...
  for frame in track_iter_progress((video_reader, len(video_reader))):
    ...
    if args.unified:
      ...
      det_bboxes, keep_idx = batched_nms(
        ...
        class_agnostic=True,
        nms_cfg=dict(
          type='nms', iou_threshold=0.5, class_agnostic=True, split_thr=100000
        ),
      )

      det_bboxes = torch.cat(
        [det_bboxes, result.pred_instances.scores[keep_idx].unsqueeze(1)], dim=1
      )
  ...
  if not args.no_post:
    instances_list = filter_and_update_tracks(
      instances_list, (frame.shape[1], frame.shape[0])
    )
  ...
  if args.sam_mask:
    ...
    for idx, (frame, track_result) in tqdm.tqdm(enumerate(zip(frames, instances_list))):
      ...
      track_result[0].pred_track_instances.instances_id = track_result[
        0
      ].pred_track_instances.instances_id.to(device)
      track_result[0].pred_track_instances = track_result[0].pred_track_instances[
        (track_result[0].pred_track_instances.scores.float() > args.score_thr).to(
          device
        )
      ]
      ...
      if len(input_boxes) > 0:
        ...
        final_mask = (1 - masks.any(axis=0).astype(np.uint8)) * 255
      else:
        final_mask = np.ones((frame.shape[0], frame.shape[1]), dtype=np.uint8) * 255

      if args.save_masks_dir:
        cv2.imwrite(os.path.join(args.save_masks_dir, f'{idx:06d}.png'), final_mask)

  # if args.save_masks_dir:
  #   os.makedirs(args.save_masks_dir, exist_ok=True)
  #   for idx, (frame, track_result) in tqdm.tqdm(enumerate(zip(frames, instances_list))):
  #     if hasattr(track_result[0].pred_track_instances, 'masks'):
  #       masks = track_result[0].pred_track_instances.masks
  #       final_mask = (1 - masks.any(axis=0).astype(np.uint8)) * 255
  #     else:
  #       final_mask = np.ones((frame.shape[0], frame.shape[1]), dtype=np.uint8) * 255
  #     cv2.imwrite(os.path.join(args.save_masks_dir, f'{idx:06d}.png'), final_mask)
