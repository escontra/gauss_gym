...
def rectangular_prism(
  init_position, width, height, up_axis, color, slice_direction: Optional[str] = None
):
...
def load_arkit_data(
  ...
  timestap_strings = [
    f'{prefix}_{np.round(timestamp, 4):.3f}' for timestamp in traj_poses[:, 0]
  ]
  transforms = []
  ...
  print(
    f'Loaded {len(depth_files)} depth files, {len(color_files)} color files, and {len(intrinsics_files)} intrinsics files'
  )
...
def load_ns_optimized_traj(splatfacto_dir: Path, transforms: List[dict]):
  ...
  pose_adjustment = torch.load(ckpt)['pipeline'][
  ...
  with open(splatfacto_dir / 'dataparser_transforms.json', 'r') as f:
    ...
    dataparser_transform_matrix = (
      torch.from_numpy(dataparser_transform.as_matrix()).float().to('cuda')
    )
    dataparser_transform_inverse_matrix = (
      torch.from_numpy(dataparser_transform.inverse().as_matrix()).float().to('cuda')
    )
  ...
  for i, transform in enumerate(transforms):
    ...
    c2ws = (
      torch.from_numpy(
        vtf.SE3.from_rotation_and_translation(
          rotation=vtf.SO3.from_matrix(c2ws[:3, :3]), translation=c2ws[:3, 3:].squeeze()
        ).as_matrix()[None]
      )
      .float()
      .to('cuda')
    ...
    c2ws[:, :3, 3] /= dataparser_scale
    c2ws = torch.bmm(
      dataparser_transform_inverse_matrix.repeat(c2ws.shape[0], 1, 1), c2ws
    )
...
def preprocess_transforms_grandslam(
  ...
  for frame in transforms:
    ...
    depth_file_path = str(
      json_path.parents[0] / frame['file_path'].replace('rgb', 'depth')
    )
...
def preprocess_transforms_arkit(
  ...
  transforms = load_arkit_data(traj_path, depth_path, color_path, intrinsics_path)
...
def get_mesh_at_frac_from_pcd(
  ...
  bbox_start_idx = int(
    (max_bbox_start_idx - min_bbox_start_idx) * frac + min_bbox_start_idx
  )
  ...
  if (end_idx - start_idx) < min_poses_per_segment:
    raise ValueError(f'Segment {frac} only has {1 + (end_idx - start_idx)} poses.')
  ...
  mesh = o3d.geometry.TriangleMesh.create_from_point_cloud_ball_pivoting(
    curr_pcd, radii=o3d.utility.DoubleVector(radii)
  )
  ...
  if decimation_factor > 1:
    mesh = mesh.simplify_quadric_decimation(len(mesh.triangles) // decimation_factor)
  if sharpen_mesh:
  ...
  mesh, curr_camera_positions, curr_camera_orientations = rotate_mesh_and_cameras(
    mesh,
  ...
  if visualize:
    ...
    colors = np.zeros_like(np.array(curr_camera_positions))  # All black for example
...
def rotate_mesh_and_cameras(mesh, camera_positions, camera_orientations, rotation):
...
def estimate_num_slices(
  ...
  for i in range(1, len(camera_positions)):
    total_distance += _distance(camera_positions[i], camera_positions[i - 1], up_axis)
  slice_distance = slice_distance + (2 * buffer_distance)
  num_slices = np.ceil(
    (total_distance - slice_overlap) / (slice_distance - slice_overlap)
  )
...
def main(_):
  ...
  load_dir = Path(os.path.expandvars(config.load_dir)).expanduser()
  ...
  if config.visualize:
    camera_point_cloud = o3d.geometry.PointCloud()
    camera_point_cloud.points = o3d.utility.Vector3dVector(np.array(camera_positions))
  ...
  for i in tqdm(range(0, num_slices)):
    progress = float(i) / (num_slices - 1) if num_slices > 1 else 0.0
