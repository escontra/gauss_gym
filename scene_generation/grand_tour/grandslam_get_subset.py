...
CAMERA_MODEL_MAP = {'hdr': 'OPENCV', 'zed': 'OPENCV'}
...
def main(argv=None):
  ...
  parser.add_argument(
    '--exclude_cameras', nargs='+', default=[], help='List of cameras to exclude'
  )
  parser.add_argument(
    '--scene_start', type=float, default=0.0, help='Where to start indexing frames'
  )
  parser.add_argument(
    '--scene_end', type=float, default=1.0, help='Where to end indexing frames'
  )
  parser.add_argument(
    '--dist_thresh',
    type=float,
    default=0.3,
    help='Distance threshold for skipping frames [m]',
  )
  parser.add_argument(
    '--rot_thresh',
    type=float,
    default=0.1,
    help='Rotation threshold for skipping frames [rad]',
  )
  ...
  max_idx = min(
    int(parsed.scene_end * len(camera_map[anchor_camera])),
    len(camera_map[anchor_camera]) - 1,
  )
  ...
  while cam_idx < max_idx:
    ...
    if prev_anchor_frame is not None:
      ...
      prev_transform = vtf.SE3.from_matrix(
        np.array(prev_anchor_frame['transform_matrix'])
      )
    ...
    for camera_name in valid_cameras:
      ...
      if 'zed' in camera_name:
        frame['depth_file_path'] = frame['depth_file_path'].replace(
          'depth', 'stereo_depth'
        )
      num_added += 1
  ...
  filename = f'{json_path.stem}_subset_{parsed.scene_start}_{parsed.scene_end}'.replace(
    '.', 'p'
  )
