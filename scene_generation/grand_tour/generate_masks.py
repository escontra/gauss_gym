...
from .utils import s5cmd_cp, s3_directory_exists  # noqa: E402

TOLERANCE = 20


def generate_masks(mission_folder: pathlib.Path, image_tags: List[str]):
  ...
  for image_tag in tqdm.tqdm(image_tags, desc='Image Tags'):
    ...
    for image_id in tqdm.tqdm(
      range(0, mission_root[image_tag]['sequence_id'].shape[0]),
      desc=f'Images ({image_tag})',
      leave=False,
      mininterval=None,
      maxinterval=None,
      miniters=None,
      ...
      mask_file_path = (
        mission_folder / 'images' / (image_tag + '_mask') / f'{image_id:06d}.png'
      )
...
def generate_masks_batch(
  mission_folder: pathlib.Path,
  image_tags: List[str],
  batch_size: int = 8,
  s3_path: str = None,
):
  """Generate masks for images using batched processing for efficiency.

  Args:
      mission_folder: Path to the mission folder
      image_tags: List of image tag names to process
      batch_size: Number of images to process in each batch (default: 8)
  """
  mission_root = zarr.open_group(store=mission_folder / 'data', mode='r')
  ...
  for image_tag in tqdm.tqdm(image_tags, desc='Image Tags'):
    if s3_path is not None:
      ...
      directory_exists = s3_directory_exists(
        s3_path[5:],
        os.path.join('grand_tour', mission_folder.stem, 'images', image_tag + '_mask')
        + '/',
      )
      if directory_exists:
        print(f'Masks already exist for {image_tag}, skipping...')
        continue

    mask_file_path = mission_folder / 'images' / (image_tag + '_mask')
    mask_file_path.mkdir(parents=True, exist_ok=True)

    # Get all image IDs that need processing
    total_images = mission_root[image_tag]['sequence_id'].shape[0]
    ...
    if not image_ids_to_process:
      if s3_path is not None:
        src = str(mission_folder / 'images' / (image_tag + '_mask')).rstrip('/')
        dst = os.path.join(s3_path, 'grand_tour', mission_folder.stem, 'images') + '/'
        s5cmd_cp(src, dst)
      tqdm.tqdm.write(f'All masks already exist for {image_tag}, skipping...')
      continue

    # Process images in batches
    num_batches = (len(image_ids_to_process) + batch_size - 1) // batch_size

    for batch_idx in tqdm.tqdm(
      range(num_batches), desc=f'Batches ({image_tag})', leave=False
    ):
      start_idx = batch_idx * batch_size
      end_idx = min(start_idx + batch_size, len(image_ids_to_process))
      batch_image_ids = image_ids_to_process[start_idx:end_idx]
      ...
      # Optimized image loading with better memory management
      def load_image(image_id):
        ...
        with Image.open(image_path) as pil_image:
          # Convert to RGB if needed and copy to avoid lazy loading issues
          if pil_image.mode != 'RGB':
            pil_image = pil_image.convert('RGB')
          else:
            pil_image = pil_image.copy()

          return pil_image, pil_image.size[::-1], mask_path

      # Load images (could be parallelized further if needed)
      for image_id in batch_image_ids:
        pil_image, size, mask_path = load_image(image_id)
        batch_images.append(pil_image)
        batch_sizes.append(size)  # (height, width)
        batch_paths.append(mask_path)

      # Process batch with Mask2Former - optimized processing
      inputs = processor(
        images=batch_images, return_tensors='pt', do_resize=True, do_normalize=True
      )
      # Move to GPU in one operation with non-blocking transfer
      inputs = {k: v.to(device, non_blocking=True) for k, v in inputs.items()}

      with torch.no_grad():
        outputs = model(**inputs)

      # Post-process segmentation maps for the batch
      predicted_segmentation_maps = processor.post_process_semantic_segmentation(
        outputs, target_sizes=batch_sizes
      )

      # Optimized saving with threading
      def save_mask(segmentation_map, mask_path):
        human_mask = (segmentation_map == 0).cpu().numpy()  # Person class is 0
        binary_mask = (~human_mask).astype(
          np.uint8
        ) * 255  # Convert to 0-255 range for PNG

        # Save with optimized PNG settings (minimal compression for speed)
        img = Image.fromarray(binary_mask, mode='L')
        img.save(mask_path, optimize=False, compress_level=1)

      # Use ThreadPoolExecutor for parallel saving
      with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        futures = [
          executor.submit(save_mask, segmentation_map, mask_path)
          for segmentation_map, mask_path in zip(
            predicted_segmentation_maps, batch_paths
          )
        ]
      ...
      # Clear GPU memory
      del inputs, outputs, predicted_segmentation_maps
...
#     get_closest_tf = FastGetClosestOdomToBaseTf("anymal_state_odometry", mission_root)
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(
    description='Generate masks for Grand Tour dataset images using batched processing',
  ...
  parser.add_argument(
    ...
    help="Mission name to process (e.g., '2024-11-04-10-57-34')",
  )
  ...
  parser.add_argument(
    ...
    help='Grand Tour dataset folder path containing missions',
  )
  ...
  grand_tour_folder = pathlib.Path(args.dataset_folder).expanduser()
  mission_folder = grand_tour_folder / mission
  image_tags = args.image_tags

  print(f'Processing mission: {mission}')
  print(f'Grand Tour folder: {grand_tour_folder}')
  print(f'Mission folder: {mission_folder}')
  print(f'Image tags: {image_tags}')
  print(f'Batch size: {args.batch_size}')

  # Generate masks for the specified image tags using batched processing
  print('Starting mask generation...')
  generate_masks_batch(
    mission_folder, image_tags, batch_size=args.batch_size, s3_path=args.s3_path
  )
